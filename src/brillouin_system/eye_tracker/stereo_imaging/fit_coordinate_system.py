...
def _umeyama(A: np.ndarray, B: np.ndarray, *, with_scale: bool = False) -> tuple[np.ndarray, np.ndarray, float]:
    ...
    if np.linalg.matrix_rank(Ac) < 2:
        raise ValueError("Degenerate configuration: points are collinear or identical.")
    ...
    varA = (Ac**2).sum() / N
    if varA < 1e-15:
        raise ValueError("Variance of A is ~0; cannot estimate transform.")
    s = (S.sum()) / varA if with_scale else 1.0
    t = muB - s * (R @ muA)
    return R, t, float(s)
