...
from brillouin_system.utils.fit_util import find_peak_locations, select_top_two_peaks, sort_peaks, \
...
def get_fitted_spectrum_gaussian(sline: np.ndarray, is_reference_mode: bool) -> FittedSpectrum:
    ...
    try:
        ...
        amp1, cen1, wid1, amp2, cen2, wid2, offset = sort_peaks(popt)  # reuse sorting function

        fittedSpect = _2Gaussian(pix, *popt)

        x_fit, y_fit = refine_fitted_spectrum(_2Gaussian, pix, popt, factor=10)
        ...
        fitted_spectrum = FittedSpectrum(
            ...
            x_fit_refined=x_fit,
            y_fit_refined=y_fit,
            parameters=popt,  # This key is a misnomer now, but you can rename later
