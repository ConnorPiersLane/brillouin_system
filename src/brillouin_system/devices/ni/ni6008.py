from __future__ import annotations

from contextlib import contextmanager
from dataclasses import dataclass
import time
from typing import Optional, Literal, Tuple

import nidaqmx
from nidaqmx.constants import AcquisitionType, TerminalConfiguration


@dataclass(frozen=True)
class StepEvent:
    t: float                 # wall-clock time (time.time()) when detected
    value: float             # sample value at detection
    baseline: Optional[float] = None
    delta: Optional[float] = None


class NI6008:
    """
    Simple NI USB-6008 analog-input reader (no threads).

    Typical PDA10A wiring:
      - BNC center (+) -> AI0 (AI0+)
      - BNC shell  (-) -> GND (AIGND)
      - Use RSE terminal configuration

    Usage:
        ni = NI6008(device="Dev1", ai_channel="ai0", sample_rate_hz=1000)
        with ni.streaming():
            v = ni.read_value()
            bg_mean, bg_std = ni.get_background_value(n_samples=500)
            evt = ni.wait_for_step(threshold=0.2, mode="delta", direction="rising")
    """

    def __init__(
        self,
        device: str = "Dev1",
        ai_channel: str = "ai0",
        *,
        terminal_config: TerminalConfiguration = TerminalConfiguration.RSE,
        min_val: float = -10.0,
        max_val: float = 10.0,
        sample_rate_hz: float = 1000.0,
    ):
        self.device = device
        self.ai_channel = ai_channel
        self.terminal_config = terminal_config
        self.min_val = float(min_val)
        self.max_val = float(max_val)
        self.sample_rate_hz = float(sample_rate_hz)

        self._task: Optional[nidaqmx.Task] = None

    @property
    def physical_channel(self) -> str:
        return f"{self.device}/{self.ai_channel}"

    @contextmanager
    def streaming(self):
        """Create/start a continuous task; close it on exit."""
        if self._task is not None:
            raise RuntimeError("NI6008 is already streaming")

        task = nidaqmx.Task()
        try:
            task.ai_channels.add_ai_voltage_chan(
                self.physical_channel,
                min_val=self.min_val,
                max_val=self.max_val,
                terminal_config=self.terminal_config,
            )

            task.timing.cfg_samp_clk_timing(
                rate=self.sample_rate_hz,
                sample_mode=AcquisitionType.CONTINUOUS,
                samps_per_chan=10,
            )

            task.start()
            self._task = task
            yield
        finally:
            try:
                task.stop()
            except Exception:
                pass
            try:
                task.close()
            except Exception:
                pass
            self._task = None

    def read_value(self, *, timeout_s: float = 1.0) -> float:
        """Read one sample (volts). Must be inside `with streaming():`."""
        if self._task is None:
            raise RuntimeError("Not streaming. Use `with ni.streaming():`.")
        v = self._task.read(number_of_samples_per_channel=1, timeout=float(timeout_s))
        return float(v)

    def read_block(self, n_samples: int, *, timeout_s: Optional[float] = None) -> list[float]:
        """Read N samples as a list[float]."""
        if self._task is None:
            raise RuntimeError("Not streaming. Use `with ni.streaming():`.")
        n = int(n_samples)
        if n <= 0:
            return []

        if timeout_s is None:
            # enough time to acquire the samples + margin
            timeout_s = max(1.0, n / self.sample_rate_hz + 0.5)

        data = self._task.read(number_of_samples_per_channel=n, timeout=float(timeout_s))
        # For 1 channel, DAQmx returns list[float] when n>1
        return [float(x) for x in data]

    def get_background_value(self, n_samples: int) -> Tuple[float, float]:
        """Return (mean, std) from a block of samples."""
        xs = self.read_block(n_samples)
        if not xs:
            raise RuntimeError("No samples read for background estimate")

        mean = sum(xs) / len(xs)
        var = sum((x - mean) ** 2 for x in xs) / len(xs)  # population variance
        return mean, var ** 0.5

    def wait_for_step(
        self,
        *,
        threshold: float,
        mode: Literal["high", "delta"] = "high",
        direction: Literal["either", "rising", "falling"] = "either",
        baseline_window_s: float = 0.25,
        min_baseline_samples: int = 50,
        timeout_s: Optional[float] = None,
        poll_sleep_s: float = 0.0,
    ) -> StepEvent:
        """
        Block until a threshold crossing is detected.

        mode:
          - "high": trigger when value crosses an absolute threshold (value >= threshold)
          - "delta": trigger when (value - baseline_mean) crosses threshold

        direction (only applies to mode="delta"):
          - "either": |delta| >= threshold
          - "rising": delta >= threshold
          - "falling": delta <= -threshold

        Returns a StepEvent. Must be called inside `with streaming():`.
        """
        if self._task is None:
            raise RuntimeError("Not streaming. Use `with ni.streaming():`.")

        thr = float(threshold)
        t0 = time.monotonic()

        baseline_n = max(1, int(self.sample_rate_hz * float(baseline_window_s)))
        # Keep a small rolling baseline buffer
        baseline_buf: list[float] = []

        while True:
            if timeout_s is not None and (time.monotonic() - t0) >= timeout_s:
                raise TimeoutError("No step detected before timeout")

            v = self.read_value(timeout_s=1.0)

            if mode == "high":
                if v >= thr:
                    return StepEvent(t=time.time(), value=v)

            elif mode == "delta":
                baseline_buf.append(v)
                if len(baseline_buf) > baseline_n:
                    baseline_buf.pop(0)

                if len(baseline_buf) < max(min_baseline_samples, min(10, baseline_n)):
                    # still filling baseline
                    if poll_sleep_s > 0:
                        time.sleep(poll_sleep_s)
                    continue

                baseline = sum(baseline_buf) / len(baseline_buf)
                delta = v - baseline

                hit = False
                if direction == "either":
                    hit = abs(delta) >= thr
                elif direction == "rising":
                    hit = delta >= thr
                elif direction == "falling":
                    hit = delta <= -thr
                else:
                    raise ValueError("direction must be 'either', 'rising', or 'falling'")

                if hit:
                    return StepEvent(t=time.time(), value=v, baseline=baseline, delta=delta)

            else:
                raise ValueError("mode must be 'high' or 'delta'")

            if poll_sleep_s > 0:
                time.sleep(poll_sleep_s)


if __name__ == "__main__":
    ni = NI6008(device="Dev1", ai_channel="ai0", sample_rate_hz=1000)

    with ni.streaming():
        vals = ni.read_block(100)
        print(f"Read {len(vals)} samples:")
        print(vals)
