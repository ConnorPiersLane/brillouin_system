...
def _2dho_binned(x, amp1, cen1, amp2, cen2, omega0, gamma, offset):
    """
    Joint two-peak DHO model with shared omega0 and gamma.
    ...
    hump of the Rayleigh order to its left (anchor at cen1 - u_pk), the right
    peak is the anti-Stokes hump of the next order (anchor at cen2 + u_pk).

    Parameter order:
        [amp1, cen1, amp2, cen2, omega0, gamma, offset]
    """
    u_pk = dho_peak_offset(omega0, gamma)
    rayleigh_left = cen1 - u_pk
    rayleigh_right = cen2 + u_pk
    return (
        _dho_pixel_integrated(x, amp1, rayleigh_left, omega0, gamma)
        + _dho_pixel_integrated(x, amp2, rayleigh_right, omega0, gamma)
        + offset
    )
