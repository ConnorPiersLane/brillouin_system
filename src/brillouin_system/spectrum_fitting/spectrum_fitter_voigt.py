...
def _1lorentzian_binned(x, amp, cen, wid, offset, slope):
    x0 = np.mean(x)
    return (
        _lorentzian_pixel_integrated(x, amp, cen, wid)
        + offset
        + slope * (x - x0)
    )


def _2lorentzian_binned(x, amp1, cen1, wid1, amp2, cen2, wid2, offset, slope):
    x0 = np.mean(x)
    return (
    ...
    def fit(self, px: np.ndarray, sline: np.ndarray, is_reference_mode: bool) -> FittedSpectrum:
        ...
        if n_peaks == 1:
            if requested_model in ("lorentzian", "lorentzian_window"):
                ...
                p0 = [amp[0], cen[0], wid[0], offset, 0.0]
                bounds = (
                    [0, x_min, 1e-12, 0, -np.inf],
                    [np.inf, x_max, x_span / 2, np.inf, np.inf],
                )
            ...
            if requested_model in ("lorentzian", "lorentzian_window"):
                ...
                p0 = [amp[0], cen[0], wid[0], amp[1], cen[1], wid[1], offset, 0.0]
                bounds = (
                    [0, x_min, 1e-12, 0, x_min, 1e-12, 0, -np.inf],
