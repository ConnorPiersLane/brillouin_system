...
the Brillouin peaks -- the structure a free linear baseline would otherwise
absorb.
...
(the 'prmr' preset / 'reflection' background in the spectrum fitter). A
per-peak scale was tested 2026-08-20 and rejected: freeing s on each side
removes the S-side constraint on the scale and re-opens the
amplitude<->centre trade (splits +3..+4 MHz on wide glycerol). An envelope
change after realignment is an instrument-state property -- verify or retake
the template, never free the fit. This module supplies R:
...
* A different ROI needs no bookkeeping: a 4-peak 200-px template renders
...
the mirror of what the fitter does on the sample side. Any residual
...
class ReflectionBackgroundMapper:
    ...
    Takes the session's calibration (a CalibrationCalculator, its parameters,
    or a (freq_left, freq_right) coefficient pair) and the row count of the
    session's sline. render(px) then returns the background on that pixel
    axis, ready to pass to the spectrum fitter as measured_background
